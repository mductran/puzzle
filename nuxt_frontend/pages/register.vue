<template>
  <v-card elevation="0" class="page-container">
    ...
    <v-form lazy-validation>
      ...
      ></v-text-field>
      <br />
      <v-btn class="mr-4" @click.stop="validate"> Register </v-btn>
    </v-form>
  </v-card>
</template>
...
const containSpecialChars = helpers.regex(
  "containSpecialChars",
  /(?![<>$^])\W|_/
);
...
export default {
  ...
  computed: {
    ...
    password1Errors() {
      ...
      !this.$v.password1.containUpper &&
        errors.push("Your password must contain at least one uppercase letter");
      !this.$v.password1.containLower &&
        errors.push("Your password must contain at least one lowercase letter");
      !this.$v.password1.containSpecialChars &&
        errors.push(
          "Your password must contain at least one special characters"
        );
      !this.$v.password1.containDigit &&
        errors.push("Your password must contain at least one digit");
      this.$v.password1.containIllegalChars &&
        errors.push(
          "Your password must not contain any illegal characters <>$%"
        );
      return errors;
    },
    password2Errors() {
